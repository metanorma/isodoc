...
require_relative "postprocess_footnote"
...
module IsoDoc::HtmlFunction
  module Html
    ...
    def html_cleanup(xml)
      xml = term_header(html_footnote_filter(html_preface(htmlstyle(xml))))
      mathml(a_name_insert(footnote_format(footnote_backlinks(html_toc(xml)))))
    end
    ...
    def authority_cleanup1(docxml, klass)
      ...
      auth&.xpath(".//h1[not(text())] | .//h2[not(text())]")&.each do |h|
        h.remove
      end
    ...
    def html_cover(docxml)
      ...
      d.children.first.add_previous_sibling populate_template(
        doc.to_xml(encoding: "US-ASCII"), :html
      )
    end
