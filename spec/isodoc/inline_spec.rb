...
RSpec.describe IsoDoc do
  ...
  it "processes identifier" do
    ...
    html_p = Nokogiri::XML(html_output)
      .at("//p[@id = 'A']").to_xml
    expect(strip_guid(html_p))
    ...
    wordxml = Nokogiri::HTML4(word)
    word_p = wordxml.at("//p").to_xml
    expect(strip_guid(word_p))
  ...
  it "processes eref content pointing to reference with citation URL" do
    ...
    html_output = IsoDoc::HtmlConvert.new({})
      .convert("test", pres_output, true)
    html_div = Nokogiri::XML(strip_guid(html_output))
      .at("//div[h1/@class='ForewordTitle']").to_xml
    expect(html_div)
      .to be_html5_equivalent_to html
    word_output = IsoDoc::WordConvert.new({})
      .convert("test", pres_output, true)
    word_div = Nokogiri::XML(strip_guid(word_output))
      .at("//div[h1/@class='ForewordTitle']").to_xml
    expect(word_div)
  ...
  it "processes eref content pointing to hidden bibliographic entries" do
    ...
    word_output = IsoDoc::WordConvert.new({})
      .convert("test", pres_output, true)
    word_p = Nokogiri::XML(word_output)
      .at("//p[@id = 'A']").to_xml
    expect(strip_guid(word_p))
      .to be_html4_equivalent_to word
  end
  ...
  it "processes variant" do
    ...
    pres_output = IsoDoc::PresentationXMLConvert
      .new(presxml_options)
      .convert("test", input, true)
    pres_clean = pres_output
      .sub(%r{<localized-strings>.*</localized-strings>}m, "")
    expect(strip_guid(pres_clean))
